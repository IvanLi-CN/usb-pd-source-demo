...
### 3.2 符合要求的候选（来自 docs/datasheets/mosfets/README.md）

- NCEP40T14G（DFN5×6，40 V）：`RDS(on)` 1.6 mΩ@10 V、2.3 mΩ@4.5 V；效率优先，适合 Buck 高边“常导通”。
- NCEP6090AGU（DFN5×6，60 V）：`RDS(on)` 2.8 mΩ@10 V、3.5 mΩ@4.5 V；裕量优先，600 kHz 下可接受的导通损耗。
- NCEP6080AG（DFN5×6，60 V）：`RDS(on)` <4 mΩ@10 V（typ 3.5）、<5 mΩ@4.5 V；作为 60 V 等效备选。
...
### 3.6 价格与 BOM 候选（按封装规格）

以下价格来自项目库索引（采集自优信电子/淘宝，日期以索引标注为准，仅作参考）。

- 5×6 方案（正式，12–24 V 母线）
  - 候选清单与价格
    - NCEP6090AGU（60 V，DFN5×6）— 3.5 mΩ@4.5 V；参考价 ¥1.96/pcs（Taobao）；特点：裕量优先，导通损耗适中，适配 DEMO/现场浪涌。
    - NCEP6080AG（60 V，DFN5×6）— <5 mΩ@4.5 V（typ 5）；参考价 ¥1.67/pcs（Taobao）；特点：成本优先，性能接近 6090AGU。
    - NCEP40T13GU（40 V，DFN5×6）— 2.3 mΩ@10 V；参考价 ~$0.29@100+（LCSC）；特点：效率优先，成本适中，需更强尖峰抑制；4.5 V 指标以数据手册实测为准。
    - NCEP4090GU（40 V，DFN5×6）— 2.2 mΩ@10 V；参考价 ~$0.32@50+（LCSC）；特点：效率优先，导通更低，需更强尖峰抑制；4.5 V 指标以数据手册实测为准。
    - NCEP4045GU（40 V，DFN5×6）— 8.5 mΩ@4.5 V（20 A）；参考价 ~$0.24@50+（LCSC）；特点：低价位，导通较高，宜用于 100 W 或低侧位的成本版对照。
    - NCEP01ND35AG（100 V，DFN5×6）— 27 mΩ@4.5 V；参考价 ¥2.16/pcs（Taobao）；不建议：导通损耗过大（仅高压特殊场景）。
  - 优选组合（鲁棒优先）：四位统一 NCEP6090AGU（60 V）
    - 理由：24 V 母线更稳健；600 kHz 下效率损失相对 40 V 方案仅小幅增加；价格中等。
  - 次选组合（效率/成本折中）：四位统一 NCEP40T13GU（40 V）
    - 前提：母线尖峰经 TVS/RC/布局抑制，SW/Drain 尖峰不过 36–38 V；以 4.5 V RDS(on) 实测结果复核损耗；若对浪涌不确定，改用 NCEP6080AG（60 V，低价版）。
  - 注：极致低价对照可选 NCEP4045GU（40 V）用于 100 W 样机或低侧位。

- 3.3×3.3 方案（12 V 母线限定 DEMO）
  - 候选清单与价格
    - NCEP3065QU（30 V，3.3×3.3‑8L）— 3.0 mΩ@4.5 V；参考价 ¥1.81/pcs；特点：RDS(on) 最低，效率更好。
    - NCEP3040Q（30 V，DFN3.3×3.3）— 9.5 mΩ@4.5 V；参考价 ¥0.61/pcs；特点：价格最低，RDS(on) 中等。
    - NCE3025Q（30 V，DFN3.3×3.3）— 14 mΩ@4.5 V；参考价 ¥0.76/pcs；特点：更低成本但导通更高。
    - NCE3035Q（30 V，DFN3.3×3.3）— 11 mΩ@4.5 V；参考价 ¥0.62/pcs；特点：价格低，指标介于二者之间。
  - 优选组合（效率优先）：四位统一 NCEP3065QU（30 V）
    - 条件：仅限 12 V 输入 DEMO、SW/Drain 尖峰 <24–25 V；布局/抑浪涌/栅阻按 600 kHz 要求执行。
  - 次选组合（成本优先）：四位统一 NCEP3040Q（30 V）
    - 条件：同上；需关注高温 RDS(on) 增加带来的导通损耗与温升。
...
- 阈值与电阻值：平均限流参考约 `40 mV`，近似 `ILIM ≈ 40 mV / RCS2`。
  - 目标 5.5 A ⇒ `RCS2 ≈ 40 mV / 5.5 A ≈ 7.3 mΩ`。
  - 可选标准值及对应限流（典型）：`7.0 mΩ → ≈5.7 A`，`7.5 mΩ → ≈5.3 A`，`6.8 mΩ → ≈5.9 A`。
  - 功耗校核：`P ≈ I²·R`，5.5 A@7 mΩ→≈0.21 W；推荐 ≥0.5 W 合金电阻（1206/2512），并预留降温铜皮。
...
- 与 SW2303 协同：SW2303 口端限流配置为 5 A；PL5501 设定约 5.5 A 作为电源级上限，避免双环冲突（量产前按瞬态/热校准微调 `RCS2`）。

## 7. 电容设计（100 W 基线｜输入/功率级/检流/输出）

说明：本设计不启用输入侧限流（`CSP1/CSN1` 短接禁用，见 6.1），因此“输入检流电阻”区域不做电容配置。以下覆盖其余四个区域：输入、功率级、输出检流电阻、输出。所有推导与方法以 PL5501 数据手册典型应用与纹波公式为依据。

1) 输入（VIN）去耦/储能
- 目标：抑制高频注入电流与母线低频纹波，满足效率与 EMI 要求（依据：输入/输出旁路电容布局指南，`docs/datasheets/pl5501-datasheet.md:11.1 Guideline`）。
- 组成：
  - 高频陶瓷（紧贴 PL5501 功率级/VIN 铜皮）：≥ 2× `22 µF/50 V` X7R，并并联 `0.1 µF`；多颗并联降低 ESL/ESR。
  - 母线储能（靠近输入接口）：`220–470 µF/35–50 V` 低 ESR 聚合物/电解（纹波电流额定满足 ≥ 输入纹波 RMS）。
- 设计提示：
  - 12→20 V（Boost）时输入电流较大（≈8.8 A@100 W），建议将陶瓷组与大电容分层分区放置；陶瓷距 MOSFET/VIN 引脚最短回路，母线大电容置于接口侧抑制低频。

2) 功率级本体去耦/自举/控制
- 目标：保证驱动与控制电源稳定，降低开关沿注入；与典型应用一致（见引脚功能：`VCC/BSTx/LDO/VREF/IREF/COMP`，`docs/datasheets/pl5501-datasheet.md:32`）。
- 组成与推荐值：
  - `VCC`（6.6 V 驱动电源）：`1 µF + 0.1 µF` X7R 至 PGND/AGND（紧贴芯片）；
  - `BST1/BST2`（自举）：各 `0.1–0.22 µF/16 V` X7R，自 `BSTx` 到 `SWx`（贴近引脚，缩短回路）；
  - `LDO`（5 V/55 mA）：`1–4.7 µF` X7R 至 GND（按负载与瞬态选择；如仅作系统待机电源，1 µF 起步）；
  - `VREF/IREF` 参考：各 `0.1 µF` 至 AGND（降低参考噪声耦合）；
  - `COMP`/补偿网络：按环路设计放置（Type‑II 起步，带宽≈fs/30），电容贴近引脚；
  - 可选 `SW‑GND` RC 缓冲：`C=330 pF–1 nF`（配合 `R=1–5 Ω`）以抑制尖峰与振铃（视波形实测决定是否装配）。

3) 输出检流电阻（RCS2 @ CSP2/CSN2）滤波
- 目标：降低开关噪声、稳定平均电流环（依据：建议在 `CSPx/CSNx` 前加入低通滤波，`docs/datasheets/pl5501-datasheet.md:190`）。
- 拓扑：`RF + CF` 一阶低通。起步值建议 `RF = 100–330 Ω`、`CF = 1–2.2 nF`（fc ≈ 1/(2πRFCF) ≈ 480 k–720 kHz），既可滤除尖峰又不过度拖慢环路；具体值以环路与负载瞬态实测整定。
- 版图：`RF/CF` 紧贴 `CSP2/CSN2` 放置，并将 `CF` 接至 AGND 的安静参考面；采样走线等长对称、远离 `SW/BST/HG/LG`。

4) 输出（VBUS）去耦/储能
- 目标：在 Buck 模式满足 `ΔVOUT ≤ ΔIL·(ESR + 1/(8·f·COUT))`（见纹波公式，`docs/datasheets/pl5501-datasheet.md:In buck mode...`），在 Boost 模式因输出电流不连续需额外考虑体电容与 ESR（同页“Boost region”说明）。
- 组成：
  - 高频陶瓷（紧贴功率级/电感输出端）：≥ 2× `22 µF/25 V` X7R 并 `0.1 µF`；
  - 低 ESR 聚合物储能（靠近接口/线缆侧）：`220–330 µF/25 V`（ESR ≤ 20–30 mΩ 级别）；
- 估算与起步：以 `fs=600 kHz`、`L=4.7 µH`、`ΔIL≈1.7 A`，若仅按电容项限制 `ΔV` 到 ≈`100 mV`，有 `COUT ≥ ΔIL/(8·f·ΔV) ≈ 1.7/(8·6e5·0.1) ≈ 3.5 µF`；考虑 Boost 模式与 ESR 项，实际应采用“多颗陶瓷并联 + 一颗聚合物”的组合起步，并用实测纹波/过冲校核。
- 版图：陶瓷电容须紧贴功率级回路（电感/低侧 MOSFET/PGND）形成最小环路；聚合物靠近接口抑制线缆与负载跃变。
