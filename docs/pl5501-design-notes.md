...
> 本节前置给出 MOSFET 与主电感在“两种封装尺寸”下的“优选/次优/混搭”结论。详细计算、边界与波形要点见后文对应小节。

### 0.1 MOSFET（四开关 Buck‑Boost 外置功率级）

- 封装A（5×6 mm｜DFN/PowerPAK/LFPAK56）
  - 优选：NCEP6090AGU ×4（60 V，鲁棒优先，适配 24 V 母线与尖峰裕量）
  - 次优：NCEP6080AG ×4（60 V，成本优先）
  - 混搭（效率/鲁棒折中）：Buck 高边 NCEP40T14G（40 V，低 RDS），其余位 NCEP6090AGU（60 V）；仅在备货允许且尖峰受控的场景使用

- 封装B（DFN‑8 3.3×3.3 mm｜实验限定，≤12 V 母线）
  - 适用边界：库内 3×3 皆为 30 V 等级；仅用于“12 V 严格受控（尖峰 <24–25 V）”的试验，不进入 24 V 正式 BOM
  - 优选：NCEP3065QU ×4（低 RDS，效率对照）
  - 次优：AON7508 ×4 或 NCEP3040Q/NCE3035Q 组合（对比 Qg/损耗/温升）
  - 混搭：Buck 高边 NCEP3065QU，其余位 AON7508（以较低 Qg 减轻驱动损耗），需配合波形/温升实测

### 0.2 主电感（两种封装，含不同感值对比）

- 封装A（≈12.5×12.5 mm 等级）
  - 优选：HPC1265‑4R7MT（4.7 µH，低 DCR，Isat/Idc 满足 100 W 热预算）
  - 次优：HPC1250‑4R7MT（薄型版本，DCR 更高，热预算需复核）

- 封装B（≈10×10 mm 等级｜叠加焊盘实验）
  - 优选：10×10 mm 级屏蔽电感 3.3 µH（Isat ≥20–25 A，DCR ≤8–10 mΩ）
  - 次优：10×10 mm 级屏蔽电感 4.7 µH（Isat/Idc 满足 100 W，DCR 略高；动态/纹波权衡）
  - 焊盘：建议采用“双封装叠加”与导热过孔矩阵以一板验证两档封装（见第 8 章）

- 感值对比（f≈600 kHz）
  - 4.7 µH：纹波/EMI 更优，Isat/Idc 更易满足，是 100 W 的默认基线
  - 3.3 µH：体积与动态更好，但纹波/峰值/铜损上升；需要加大输出电容并关注 EMI
...
- 本域仅 SW2303 占用 I2C（`SDA_PL/SCL_PL`），无默认地址冲突。
- 若未来引入可配置地址的从设备，请沿用“先隔离、后改址、再并总线”的流程（可参考 SW7201 章节的隔离位做法），并在 BOM 中预留 0 Ω/DNP 串联位以降低调试风险。
