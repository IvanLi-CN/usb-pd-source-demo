...
#### 5.3.2 输出—口端侧抑振/去耦（位于 VBUS 检流电阻外侧，靠 Type‑C 口）

- 容量与组成：占 `20–30%`；以 MLCC 为主，例如 `2×10–22 µF / 25–50 V X7R`。如确需，可加 `≤100 µF / 25–35 V` 小体量聚合物。
- 注意：口端侧电容在插接/阶跃时的充放电电流会计入“负载电流”，不宜放置过大的聚合物容量；若选 25 V，需配合 TVS/吸收并经波形确认峰值＜额定。
- 目的：抑制连接器与线缆引起的局部振铃，改善端口测得的纹波。
