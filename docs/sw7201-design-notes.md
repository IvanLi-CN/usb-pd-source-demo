...
目标：避免上电即出现 0x3C 地址冲突（SW7201 默认 0x3C，SW2303 也为 0x3C）。采取“先隔离、后改址、再并总线”的流程：

硬件隔离选项（择一即可）
- 总线隔离：在 SW2303 的 `SDA_SW`/`SCL_SW` 串联 0 Ω 可配置位（默认断开/DNP），或使用小型模拟开关，上电阶段先断开与总线的连接。
- 供电隔离：通过负载开关或跳线，先不给 SW2303 上电；待 SW7201 改址完成后再上电。

主机初始化时序（上电后）
1) 确保 SW2303 与总线断开（或未上电），总线上仅 SW7201 应答。
2) 主机以默认地址 `0x3C` 通信，写入 `REG 0x1A[1:0]=01` 将 SW7201 改为 `0x38`；随后 I2C 扫描确认 `0x38` 有 ACK。
3) 重新接入 SW2303（闭合 0 Ω/打开模拟开关/上电），此时总线设备为 `0x38`（SW7201）与 `0x3C`（SW2303）。
4) 后续冷启动如需再次改址，重复上述步骤；或在硬件上永久保留总线隔离位，作为产测/量产初始化流程的一部分。

补充：本板 I2C 上拉由外部主机提供（板上无上拉），请在改址前确认上拉与时钟频率设置正确，先进行一次总线扫描再下发寄存器写入。
