...
仅通过上电时序完成 SW7201 改址，避免与 SW2303 冲突：

1) 上电阶段只给 SW7201 供电，SW2303 暂不供电（不接通 VDD）。
2) 主机以默认地址 `0x3C` 与 SW7201 通信，写入 `REG 0x1A[1:0]=01` 改为 `0x38`，并以 I2C 扫描确认 `0x38` 有 ACK。
3) 为 SW2303 上电。此时总线设备为 `0x38`（SW7201）与 `0x3C`（SW2303）。
4) 冷启动需要再次改址时，重复上述上电顺序。

注：本域 I2C 上拉由外部主机提供，请在第 2 步前确认上拉与总线频率配置正确。
...
- VBUS 侧（CSN_VBUS / CSP_VBUS）
  - 跨检流电容（100 nF）：靠近检流电阻本体，直接并在分流电阻两端。
  - 分流→CS 引脚的 RC 低通（串联小电阻、各自到地小电容）：靠近 SW7201 芯片的 CSP_VBUS/CSN_VBUS 引脚放置。
  - 依据：docs/datasheets/sw7201-datasheet.md:58（明确“分流两端并 100 nF，且分流与 CS 之间加低通”）。

- VBAT 侧（CSN_VBAT / CSP_VBAT）
  - 分流→CS 引脚的 RC 低通：靠近 SW7201 芯片的 CSP_VBAT/CSN_VBAT 引脚放置。
  - 跨检流电容：手册未要求在 VBAT 侧强制并联“跨分流电容”。
  - 依据：通用布局与采样走线指引 docs/datasheets/sw7201-datasheet.md:265（Kelvin、等长、远离噪声节点）。

- 共性：CSP/CSN 采用 Kelvin 等长对称差分走线，远离 BST/SW/HG/LG 等高速节点；采样地单点回 AGND。
